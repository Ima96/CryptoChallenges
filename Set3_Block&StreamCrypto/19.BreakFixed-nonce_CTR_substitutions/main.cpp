...
string breakFixedNonceCTR(string* encr_str_set, int set_len);
...
int main(){
	...
	file.close();
	unsigned char** u_decoded_in = new unsigned char*[line_count];
	string encrypted_in[line_count], decoded_in;
	for(int i=0; i<line_count; i++){
		u_decoded_in[i] = new unsigned char[input[i].length()];
		decoded_in = base64Decode(input[i], u_decoded_in[i]);
		cout << decoded_in << endl;
		encrypted_in[i] = enc_dec_CTR(decoded_in, key, nonce);
	}

	string obtained_key = breakFixedNonceCTR(encrypted_in, line_count);
...
string breakFixedNonceCTR(string* encr_str_set, int set_len){
	...
	string temp1, temp2;
	unsigned char keystream1, keystream2;
	int current_len, min_len = 1000;

	for(int i=0; i<set_len; i++){
		current_len	= encr_str_set[i].length();
		if(current_len<min_len) min_len = current_len;
	}
	cout << "Minimum length is: " << min_len << endl;

	for(int i=0; i<set_len; i++){
		encr_str_set[i] = encr_str_set[i].substr(0, min_len);
	}

	string byte_block[set_len];
	for(int i=0; i<set_len; i++){
		/* STILL TO BE DONE */	
	}
